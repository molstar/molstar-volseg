...
class CategoryWriter_VolumeData3dInfo(CategoryWriter):
    def __init__(self, ctx: IPreprocessedMetadata, count: int, category_desc: CategoryDesc):
        self.data = ctx
        self.count = count
...
class CategoryWriterProvider_VolumeData3dInfo(CategoryWriterProvider):
    def _decide_encoder(self, ctx: IPreprocessedMetadata) -> tuple[BinaryCIFEncoder, np.dtype]:
    ...
    def category_writer(self, ctx: IPreprocessedMetadata) -> CategoryWriter:
        field_desc: list[FieldDesc] = Fields_VolumeData3dInfo(*self._decide_encoder(ctx)).fields
