...
_DELTA_RLE_ENCODER = encoder.ComposeEncoders(
    encoder.DELTA, encoder.RUN_LENGTH, encoder.BYTE_ARRAY
)
...
def coord_encoder(coords: np.ndarray) -> BinaryCIFEncoder:
    ...
    return encoder.ComposeEncoders(
        encoder.IntervalQuantization(
            coords.min(), coords.max(), num_steps, array_type=array_type
        ),
        encoder.DELTA,
...
def decide_encoder(
    ctx: np.ndarray, data_name: str
) -> tuple[BinaryCIFEncoder, np.dtype]:
    ...
    if data_type == DataTypeEnum.Float32 or data_type == DataTypeEnum.Float64:
        ...
        interval_quantization = encoder.IntervalQuantization(
            data_min, data_max, 255, DataTypeEnum.Uint8
        )
