...
class RealVolumeToFakeSFFConverter:
    ...
    def create_fake_sff_from_input_volumes(self, input_volumes_filepaths: List[Path], output_sff_dir: Path,
        ...
        for filepath in input_volumes_filepaths:
            entry_name = self.convert_map_filename_to_entry_name(filepath.stem)
            ...
            segm_grid_and_ids = self.create_fake_segmentation_from_real_volume(filepath, number_of_segments)
            ...
            self.write_fake_segmentation_to_sff(
...
if __name__ == '__main__':
    SOURCE_DB = 'emdb'
    converter = RealVolumeToFakeSFFConverter()
    converter.clear_dir_with_files_for_storing(DIR_WITH_FILES_FOR_STORING)
    list_of_input_volume_files = converter.get_list_of_input_volume_files(REAL_VOLUME_DIR)
    converter.create_fake_sff_from_input_volumes(list_of_input_volume_files, DIR_WITH_FILES_FOR_STORING, SOURCE_DB)
