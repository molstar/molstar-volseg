...
@pytest.mark.parametrize("volume_test_inputs", VOLUME_TEST_INPUTS)
def test_process_volume_metadata(volume_test_inputs: TestInput):
    with context_for_tests(volume_test_inputs, WORKING_FOLDER_FOR_TESTS) as ctx:
        ctx: TestContext
        v = get_internal_volume_from_input(
                volume_test_inputs, ctx.test_input_asset_path, ctx.working_folder
            )
        process_volume(v)
