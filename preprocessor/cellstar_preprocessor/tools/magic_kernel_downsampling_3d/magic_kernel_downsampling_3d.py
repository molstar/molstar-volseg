...
class MagicKernel3dDownsampler:
    ...
    from ._helper_methods import (
        compute_offset_indices_from_radius,
        generate_dummy_arr,
        setdiff2d_set,
    )

    def downsample_using_magic_kernel(
        self, arr: np.ndarray, kernel: Tuple[int, int, int, int, int]
    ) -> np.ndarray:
        """
        ...
        for voxel_coords in target_voxels_coords:
            inner_layer_voxel_coords = self.get_voxel_coords_at_radius(
                voxel_coords, 1, arr.shape
            )
            outer_layer_voxel_coords = self.get_voxel_coords_at_radius(
                voxel_coords, 2, arr.shape
            )
    ...
    def create_x2_downsampled_grid(
        self, original_grid_shape: tuple[int, int, int], fill_value, dtype
    ) -> np.ndarray:
    ...
    def get_voxel_coords_at_radius(
        self,
        target_voxel_coords: Tuple[int, int, int],
        radius: int,
        ...
        offset_idx_range_all_layers = (
            MagicKernel3dDownsampler.compute_offset_indices_from_radius(radius)
        )
        # indices range for offsets for just inner layers (e.g. [-1, 0, 1])
        offset_idx_range_inner_layers = (
            MagicKernel3dDownsampler.compute_offset_indices_from_radius(radius - 1)
        )

        # arr of all possible offsets if we select all layers (not just surface)
        offsets_all_layers = np.array(
            tuple(product(offset_idx_range_all_layers, repeat=ndim))
        )
        # arr of all possible offsets if we select just inner layers (except surface)
        offsets_inner_layers = np.array(
            tuple(product(offset_idx_range_inner_layers, repeat=ndim))
        )
        # arr of offsets corresponding to just surface layer (what is actually required)
        offsets_surface_layer = MagicKernel3dDownsampler.setdiff2d_set(
            offsets_all_layers, offsets_inner_layers
        )
    ...
    def compute_downsampled_voxel_value(
        ...
        values_sum = (
            k[2] * target_voxel_value
            + k[1] * inner_layer_voxel_values.sum()
            + k[0] * outer_layer_voxel_values.sum()
        )
        coefficients_sum_3d = (
            k[2] * 1
            + k[1] * inner_layer_voxel_values.size
            + k[0] * outer_layer_voxel_values.size
        )
