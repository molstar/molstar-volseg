...
def convert_tiff_to_mrc(tiff_pathes: list[Path], mrc_path: Path, data_type: Literal[1, 0] | None, pixel_spacing: list[float, float, float] | None):
    ...
    pairs_of_pathes = [(p, (folder_for_16_bit / f'{p.stem}_16bit{"".join(p.suffixes)}')) for p in tiff_pathes]
    ...
    print(f'Command: {lst}')
    api_process = subprocess.Popen(lst)
...
if __name__ == '__main__':
    ...
    tiff_pathes = _get_tiff_pathes(Path('/mnt/data_backup_ceph/datasets_from_alessio/empiar-12017/WT_fed_6461_mitochondria_instance_segmentation'))
    convert_tiff_to_mrc(tiff_pathes,
                        Path('/mnt/data_backup_ceph/datasets_from_alessio/empiar-12017/WT_fed_6461_mitochondria_instance_segmentation/volume.mrc'),
                        # 1,
                        [8.0, 8.0, 8.0])