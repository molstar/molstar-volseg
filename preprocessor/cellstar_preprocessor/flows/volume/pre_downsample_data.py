...
def pre_downsample_data(input_paths: list[str], input_kinds: list[InputKind], pre_downsample_data_factor: int, working_folder: str):
    ...
    for idx, p in enumerate(input_paths):
        ...
        if input_kinds[idx] == InputKind.tiff_image_stack_dir:
            downsized_path = Path(working_folder) / 'temp_downsized' / str(Path(p).stem + '_downsized' + Path(p).suffix) 
            downsized_pathes.append(downsized_path)
            if downsized_path.exists():
                shutil.rmtree(downsized_path)
            downsized_path.mkdir(parents=True)
            args.append((Path(p), downsized_path, pre_downsample_data_factor))
