...
def pre_downsample_data(inputs: list[RawInput], pre_downsample_data_factor: int, working_folder: str):
    ...
    for idx, i in enumerate(inputs):
        ...
        downsized_input_path = temp_downsized_folder_path / str(Path(inputs[idx].path).stem + '_downsized' + Path(inputs[idx].path).suffix)
        if inputs[idx].kind in [InputKind.tiff_image_stack_dir, InputKind.tiff_segmentation_stack_dir]:
            ...
            for input_tiff in Path(inputs[idx].path).glob('*.ti*'):
                ...
                downsize_args.append((input_tiff, output_tiff, pre_downsample_data_factor))
