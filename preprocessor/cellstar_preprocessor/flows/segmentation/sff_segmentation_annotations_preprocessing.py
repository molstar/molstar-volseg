...
def __process_raw_segment_data(time: int, segment: dict[str, Any], d: AnnotationsMetadata, segmentation_id: str, segmentation_kind: SegmentationKind):
    description_id = str(uuid4())
    target_id  = TargetId(segment_id=segment["id"], segmentation_id=segmentation_id)
    raw_external_references: list[dict[str, Any]] = segment[
...
def _processs_raw_sff_annotations(internal_segmentation: InternalSegmentation, d: AnnotationsMetadata, segmentation_id: str):
    time = 0

    for segment in internal_segmentation.raw_sff_annotations["segment_list"]:
            if segment["three_d_volume"] == None:
                d = __process_raw_segment_data(time, segment, d, segmentation_id, SegmentationKind.mesh)
            else:
                if str(segment["three_d_volume"]["lattice_id"]) == segmentation_id:
                    d =  __process_raw_segment_data(time, segment, d, segmentation_id, SegmentationKind.lattice)
...
def sff_segmentation_annotations_preprocessing(
    ...
    d: AnnotationsMetadata = AnnotationsMetadata.parse_obj(root.attrs["annotations_dict"])
    ...
    if (
        ...
        for lattice_id, lattice_gr in root[
            ...
            #         # create description
            #         description_id = str(uuid4())
            #         target_id  = TargetId(segment_id=segment["id"], segmentation_id=str(lattice_id))
            #         raw_external_references: list[dict[str, Any]] = segment[
            #             "biological_annotation"
            #         ]["external_references"]
            #         external_referneces: list[ExternalReference] = (
